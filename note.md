...
**可擦写可编程 ROM**（Erasable Programmable ROM, EPROM）：有一个透明的石英窗口，允许光到达存储单元。紫外线光照射进窗口，EPROM 单元就被清零。对它的编程（写 1）通过一种特殊设备完成。可以被擦除和重编程约 $1000$ 次。**电子可擦除 PROM**（Electrically Erasable PROM, EEPROM）不需要一个物理上独立的编程设备，EEPROM 可以被编程数万次。
...
> 对于 DRAM 和 SRAM 的容量，通常以 $K=2^{10}, M=2^{20}, G=2^{30}, T=2^{40}$ 为单位，而对于磁盘和网络等 I/O 设备的容量，通常以 $K=10^3, M=10^6, G=10^9, T=10^{12}$ 为单位。
...
磁盘以扇区大小的块读写数据，对扇区的**访问时间（access time）**主要可以分成三个部分：
...
磁盘控制器接收到读命令后，将逻辑块号翻译成扇区地址，读扇区内容并传送到主存。这一过程不需要 CPU 干涉。

设备可以自己执行读或写总线事务，而不需要 CPU 干涉的过程，称为**直接内存访问（Direct Memory Access, DMA）**。这种数据传送称为 **DMA 传送（DMA transfer）**。
...
如果要写一个已经缓存了的字 $\omega$（写命中, write hit），在高速缓存更新了它的副本之后，还需要更新下一层中它的副本。
...
**写策略**：直写容易实现，且可以使用独立于高速缓存的**写缓冲区（write buffer）**用于更新内存。写回引起的传送较少，允许更多的到内存的带宽用于执行 DMA 的 I/O 设备。层次结构越低，传送时间越长，减少传送数量越重要，越多采用写回。
...
`size` 参数越小，时间局部性越好；`stride` 参数越小，空间局部性越好。
